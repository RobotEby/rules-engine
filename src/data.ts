...
export function validateData(
  ...
  function visit(
    ...
    if (++visited > VALIDATION_LIMITS.maxValues)
      return `${path}: limite de ${VALIDATION_LIMITS.maxValues} valores excedido`;
    if (depth > VALIDATION_LIMITS.maxDepth)
      return `${path}: profundidade máxima ${VALIDATION_LIMITS.maxDepth} excedida`;
    if (
      value === null ||
      typeof value === "string" ||
    ...
    if (typeof value === "number")
      return Number.isFinite(value)
        ? undefined
        : `${path}: número deve ser finito`;
    if (value === undefined && allowUndefined) return;
    if (typeof value !== "object")
      return `${path}: valor fora do domínio ${allowUndefined ? "JSON + undefined" : "JSON"}`;
    if (types.isProxy(value)) return `${path}: proxies não são suportados`;
    if (ancestors.has(value))
      return `${path}: referência cíclica não suportada`;
    ...
    if (visited + childCount > VALIDATION_LIMITS.maxValues)
      return `${path}: limite de ${VALIDATION_LIMITS.maxValues} valores excedido`;
    if (array && value.length !== childCount)
      return `${path}: array deve ser denso e sem propriedades extras`;
    ancestors.add(value);
    for (const key of keys) {
      if (array && key === "length") continue;
      if (typeof key === "symbol")
        return `${path}: propriedades symbol não são suportadas`;
      const next = array ? `${path}[${key}]` : propertyPath(path, key);
      if (array && (!/^(0|[1-9]\d*)$/.test(key) || Number(key) >= value.length))
        return `${next}: propriedade extra em array`;
      const descriptor = Object.getOwnPropertyDescriptor(value, key)!;
      if (!("value" in descriptor))
        return `${next}: accessors não são suportados`;
      if (!descriptor.enumerable)
        return `${next}: propriedade deve ser enumerável`;
...
export function freezeData<T>(value: T, seen = new Set<object>()): T {
  if (value !== null && typeof value === "object" && !seen.has(value)) {
    seen.add(value);
    for (const key of Object.keys(value))
      freezeData(Object.getOwnPropertyDescriptor(value, key)!.value, seen);
    Object.freeze(value);
  }
  return value;
}
