...
export function requireExplanation(result: EvaluationResult, ruleId: string) {
  ...
  assert.ok(
    rule,
    `Regra "${ruleId}" não encontrada na explicação da versão ${result.rulesetVersion}`,
  );
  return rule.trace;
}
...
function printResult(label: string, result: EvaluationResult): void {
  ...
  for (const rule of result.results)
    console.log(
      `${rule.matched ? "ATENDIDA" : "NÃO ATENDIDA"} [${rule.ruleId}] ${rule.description ?? ""}`,
    );
...
export function runDemo(): void {
  ...
  const scenarios: {
    ...
    {
      label: "R$180 no Sudeste",
      facts: pedido,
      shipping: [false, true],
      discounts: [5],
    },
    {
      label: "R$250 no Centro-Oeste",
      facts: {
        ...pedido,
        valorPedido: 250,
    ...
    ...(
      ...
      label: `Limite: R$${value.toFixed(2)} no Sudeste`,
      facts: { ...pedido, valorPedido: value },
      shipping: [v1, v2] as [boolean, boolean],
      discounts: [5],
    })),
    {
      label: "R$500, primeira compra, sem destino.regiao",
      facts: { ...pedido, valorPedido: 500, quantidadeItens: 1, destino: {} },
      shipping: [false, false],
      discounts: [5],
    },
    {
      label: "Descontos de 10% e 5% retornados juntos",
      facts: { ...pedido, valorPedido: 500, quantidadeItens: 6 },
      shipping: [true, true],
      discounts: [10, 5],
    },
  ];
  ...
  for (const versionIndex of [0, 1] as const) {
    ...
    for (const scenario of scenarios) {
      ...
      assert.equal(
        result.actions.filter((action) => action.type === "FREE_SHIPPING")
          .length,
        Number(scenario.shipping[versionIndex]),
        scenario.label,
      );
      assert.deepEqual(
        result.actions
          .filter((action) => action.type === "PERCENT_DISCOUNT")
          .map((action) => action.params?.percentual),
        scenario.discounts,
        scenario.label,
      );
      assert.equal(
        result.actions.length,
        Number(scenario.shipping[versionIndex]) + scenario.discounts.length,
        scenario.label,
      );
  ...
  console.log(
    "\nExplicação da regra de frete:",
    JSON.stringify(
      requireExplanation(engine.explain(pedido), SHIPPING_RULE),
      null,
      2,
    ),
  );
