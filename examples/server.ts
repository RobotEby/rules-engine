  ...
  return new Promise((resolve, reject) => {
    ...
    const onData = (chunk: Buffer) => {
      ...
      if (bytes > MAX_BODY_BYTES) {
        fail(new HttpError(413, "Corpo excede o limite de 1 MiB"));
        return;
      }
      chunks.push(chunk);
    };
    const onEnd = () => {
      ...
      try {
        ...
        fail(new HttpError(400, "JSON malformado"));
        return;
      }
      settled = true;
      cleanup();
    ...
    const onAborted = () =>
      fail(new HttpError(400, "Leitura do corpo interrompida"));
    const onClose = () => {
...
function authorize(req: IncomingMessage, token: string | undefined): void {
  if (!token || token.trim() === "")
    throw new HttpError(
      503,
      "Administração desabilitada: configure ADMIN_TOKEN",
    );
...
export function createDemoServer(options: {
  ...
  return createServer(async (req, res) => {
    try {
      if (req.url === "/rules" || req.url?.startsWith("/rules/"))
        authorize(req, adminToken);
      if (req.method === "POST" && req.url === "/evaluate") {
        const body = await readJsonBody(req);
        const validation = validateFacts(body);
        if (!validation.valid)
          throw new FactsValidationError(validation.errors);
      ...
      if (req.method === "POST" && req.url === "/rules/rollback") {
        ...
        if (
          ...
          throw new HttpError(
            422,
            "Corpo deve conter somente version, uma string não-vazia",
          );
        }
      ...
      if (
        ...
        error instanceof FactsValidationError
      ) {
...
function main(): void {
  ...
  const initial: unknown = JSON.parse(
    readFileSync(
      new URL("./rules-frete-desconto.v1.json", import.meta.url),
      "utf8",
    ),
  );
  ...
  server.listen(port, "127.0.0.1", () => {
    ...
    console.log(
      `Servidor em http://127.0.0.1:${address.port} — ruleset v${engine.getCurrentVersion()}`,
    );
    console.log(
      "POST /evaluate; operações /rules exigem ADMIN_TOKEN e Authorization: Bearer <token>.",
    );
  });
  const shutdown = () => {
    server.close((error) => {
